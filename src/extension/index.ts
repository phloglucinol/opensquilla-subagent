...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: [
			...
			"one tightly scoped high-risk question. Pi's built-in tools remain the default",
			"for one trivial operation, but simple bounded read-only tasks may be delegated",
			"when explicitly requested, useful in parallel, or helpful for preserving parent",
			"context. Scout only when scope is unknown; use separate calls for independent",
			"checks and a chain only for dependent phases. Pi performs final synthesis by",
			"default. Each call uses an isolated OpenSquilla profile, so sibling calls in one",
			"Pi response can execute concurrently.",
		].join(" "),
		promptSnippet:
			"Delegate an isolated task to OpenSquilla with its own SquillaRouter model routing",
		promptGuidelines: [
			"Use Pi's built-in tools by default for one trivial operation. Simple bounded read-only tasks may use opensquilla_subagent when the user explicitly requests OpenSquilla, parallel sibling execution is useful, or delegation preserves the parent context; use effort=fast, permissions=restricted, and a strict output limit. Keep small edits and shared-session work in Pi.",
