...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: [
			"Delegate one bounded, self-contained task to an isolated OpenSquilla agent.",
			...
			"Use effort=fast for scouting, extraction, and narrow checks; reserve deep",
			"for one tightly scoped high-risk question. Independent review dimensions",
			"should be separate bounded calls, executed sequentially because OpenSquilla",
			"holds a profile-wide lock.",
			"NOT for: simple edits (use edit), reading files (use read), or work",
			"needing shared Pi session state. For dependent phases, use opensquilla_chain.",
		].join(" "),
		promptSnippet:
			"Delegate an isolated task to OpenSquilla with its own SquillaRouter model routing",
		promptGuidelines: [
			"Use opensquilla_subagent for one bounded objective. Split broad audits into 2-4 calls by module or review dimension, but execute them sequentially because OpenSquilla serializes a profile; use effort=fast for scouting/extraction and avoid asking one subagent to read the whole repository and perform a complete deep review.",
			"Use opensquilla_chain only when later phases depend on earlier output. Keep chain steps narrow and their handoff summaries concise so each step can route below c3 when appropriate.",
		],
	...
	pi.registerTool({
		...
		description: [
			"Run dependent OpenSquilla phases as a chain. Each step is an independent",
			"turn with separate SquillaRouter classification and visible activity updates.",
			"Use {previous} only for concise handoffs; 12KB is inserted by default.",
			"Prefer 2-4 narrow steps such as scout→focused review→synthesis.",
			"For independent review dimensions, use separate bounded calls in sequence",
			"so the parent can retain each result without one broad c3 prompt.",
		...
		promptGuidelines: [
			"Use opensquilla_chain only for 2-4 dependent phases. Make each step one bounded objective, use effort=fast for scouting/extraction, and request concise structured handoffs. Use separate sequential opensquilla_subagent calls for independent checks.",
		],
		executionMode: "sequential",
		parameters: Type.Object(
			{
